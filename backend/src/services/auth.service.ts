import { JwtTokens, LoginResponse } from '../types/types.js';
...
export class AuthService {
  ...
  async login(email: string, password: string): Promise<LoginResponse> {
    ...
    await authRepository.update({ user: user }, { refreshToken: tokens.refreshToken });
  ...
  async refresh(oldRefreshToken: string) {
    ...
    return {
      tokens,
      data: { email: userAuth.user.email, username: userAuth.user.username },
    };
  }
}
