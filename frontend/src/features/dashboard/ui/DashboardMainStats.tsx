...
export default function DashboardMainStats({ userMediaStats }: DashboardMainStatsProps) {
  const { watchedMedia, watchedEpisodes, overallRuntime, avgRating } =
    userMediaStats.aggregatedUserMediaStats;

  return (
    <Grid container spacing={3}>
      <Grid size={{ xs: 12, md: 12, lg: 6 }}>
        <DashboardCard dashStat={watchedMedia} mainColor="blue" label="Movies watched">
          <Stack
            p={3}
            borderRadius={theme.shape.borderRadiusScale.md}
            sx={{ background: theme.palette['blue'].dark }}
          >
      ...
      <Grid size={{ xs: 12, md: 12, lg: 6 }}>
        <DashboardCard dashStat={watchedEpisodes} mainColor="green" label="TV episodes">
          <Stack
            p={3}
            borderRadius={theme.shape.borderRadiusScale.md}
            sx={{ background: theme.palette['green'].dark }}
          >
      ...
      <Grid size={{ xs: 12, md: 12, lg: 6 }}>
        <DashboardCard dashStat={overallRuntime + 'm'} mainColor="purple" label="Watch time">
          <Stack
            p={3}
            borderRadius={theme.shape.borderRadiusScale.md}
            sx={{ background: theme.palette['purple'].dark }}
          >
      ...
      <Grid size={{ xs: 12, md: 12, lg: 6 }}>
        <DashboardCard dashStat={avgRating ?? 0.0} mainColor="orange" label="Avg rating">
          <Stack
            p={3}
            borderRadius={theme.shape.borderRadiusScale.md}
            sx={{ background: theme.palette['orange'].dark }}
          >
