...
async function getMoviesByPage(page = 1, minRating, genres, sortBy) {
  ...
  const URL = `${TMDB_URL}/3/discover/movie?include_adult=true&include_video=false&language=en-US&page=${page}&sort_by=${sortBy}&vote_average.gte=${minRating}${concatGenres ? `&with_genres=${concatGenres}` : ''}`;
